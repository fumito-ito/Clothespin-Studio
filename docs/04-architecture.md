...
- 変更は **コマンド関数経由**（`addPin`, `connectPin`, `setColor`, `deleteSubtree`, `rotateRoll` …）。
...
- **roll/pitch 調整**: 選択中の連結ピンにキー（例: `[` `]` で roll、`{` `}` で pitch）。pitch は丸線リング（`g4`/`g6`）接続時のみ有効。
