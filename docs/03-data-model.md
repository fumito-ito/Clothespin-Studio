...
- 色はマテリアルとして埋め込む（パレット色ごとにマテリアル分割・マージして出力サイズを抑える）。
- **座標系・単位（確定）**: glTF 慣例に合わせ **Y-up・メートル**（ドメインの Z-up・mm から変換）。
...
- **座標系・単位（確定）**: スライサの事実上の標準に合わせ **Z-up・mm のまま**。

## 5. 画像出力（PNG）— FR-IO6

- 直前に 1 フレーム描画してから canvas を `toBlob("image/png")` で書き出す
  （`preserveDrawingBuffer` 不要・確実）。
- 解像度はビューポート等倍（確定・初期実装）。2x / 4x の高解像度・背景透過はバックログ。
